...
class LandingPageModel(BaseModel):
    ...
    # Explicit user-edited version of final_text (primary landing content).
    # Distinct from Step 1 generated final_text.
    # None = not user-edited → Step 1 value is used.
    # AI must never generate this field — injected from user input only.
    final_text_override: Optional[str] = None
    # Explicit user-edited version of entry_message (messenger hook).
    # Distinct from Step 1 generated entry_message.
    # None = not user-edited → Step 1 value is used.
    # AI must never generate this field — injected from user input only.
    entry_message_override: Optional[str] = None
...
class LandingGenerateRequest(BaseModel):
    ...
    hero_title_override: Optional[str] = None
    # Explicit user-edited final_text. Overrides Step 1 generated value.
    # None = not provided → Step 1 value is used.
    final_text_override: Optional[str] = None
    # Explicit user-edited entry_message. Overrides Step 1 generated value.
    # None = not provided → Step 1 value is used.
    entry_message_override: Optional[str] = None


class LandingDraftResponse(BaseModel):
    """Response for POST /projects/{project_id}/landing/draft.
    Returns Step 1 semantic output for user review before Step 2.
    hero_title is NOT included — it is generated by Step 2, not Step 1.
    """
    final_text: str
    entry_message: str
