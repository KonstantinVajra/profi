...
export function StyleGrid({ grid: _ }: { grid: StyleGrid }) {
  // Photo loading not yet implemented. Show a neutral placeholder.
  return (
...
export function SimilarCaseBlock({ similarCase }: { similarCase: SimilarCase }) {
  ...
  return (
    <section className="mb-6 bg-gray-50 rounded-2xl p-4 border border-gray-100">
      ...
      {similarCase.title && (
        <p className="text-sm font-semibold text-gray-800">{similarCase.title}</p>
      )}
      {similarCase.description && (
        <p className="text-sm text-gray-500 mt-1 leading-relaxed">{similarCase.description}</p>
      )}
    </section>
  );
}
...
export function PriceCardBlock({ priceCard }: { priceCard: PriceCard }) {
  return (
    <section className="mb-6 border border-gray-200 rounded-2xl p-5 bg-white">
      ...
      <p className="text-3xl font-bold text-gray-900 leading-none">{priceCard.price}</p>
      <p className="text-sm text-gray-500 mt-2 leading-relaxed">{priceCard.description}</p>
    </section>
  );
}
...
export function PhotographerBlock({ photographer }: { photographer: Photographer }) {
  return (
    <section className="mb-6 flex items-center gap-3 py-4 border-t border-b border-gray-100">
      ...
      <div>
        <p className="text-sm font-semibold text-gray-900">{photographer.name}</p>
      ...
      <div className="space-y-3">
        {visible.map((r, i) => (
          <div key={i} className="bg-gray-50 rounded-xl p-4 border border-gray-100">
            ...
            {r.author && (
              <p className="text-xs text-gray-400 mt-2 font-medium">— {r.author}</p>
            )}
          </div>
        ))}
      </div>
