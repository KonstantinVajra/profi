...
const CHANNEL_STYLES: Record<string, string> = {
  telegram:  "bg-gray-900 hover:bg-gray-700 text-white",
  whatsapp:  "bg-green-600 hover:bg-green-700 text-white",
  phone:     "bg-gray-900 hover:bg-gray-700 text-white",
  instagram: "bg-gray-900 hover:bg-gray-700 text-white",
...
const CHANNEL_ICONS: Record<string, string> = { telegram: TELEGRAM_SVG, whatsapp: WHATSAPP_SVG };
...
export function CtaButtons({
  ...
  // Resolve active buttons: channel must exist in cta.channels AND have a contact value.
  // Unknown channels (not in CONTACT_URLS) are silently skipped.
  const activeButtons = cta.channels.flatMap((ch) => {
    const buildUrl = CONTACT_URLS[ch];
    if (!buildUrl) return []; // unknown channel from AI — ignore
    const value = contactInfo[ch as keyof ContactInfo]?.trim();
    if (!value) return []; // no contact value for this channel
    return [{
      channel: ch,
      href:    buildUrl(value),
  ...
  return (
    <section className="px-6 pt-6 pb-8 space-y-3 border-t border-gray-100">
      ...
      {activeButtons.map(({ channel, href, label, style, icon }) => (
        <a
          key={channel}
          href={href}
          target="_blank"
